...
// Hàm chuyển đổi giá từ "75K" thành 75000
...
const CartItem = ({ item, index, onCartUpdate }) => {
  const [quantity, setQuantity] = useState(item.quantity);
  const [total, setTotal] = useState(formatPrice(item.price) * item.quantity);
  const minQuantity = 1;
  const maxQuantity = 100;

  useEffect(() => {
    // Cập nhật total khi quantity thay đổi
    const newTotal = formatPrice(item.price) * quantity;
    setTotal(newTotal);

    // Cập nhật giỏ hàng trong localStorage
    updateCartInLocalStorage(item.productId, quantity, newTotal);
  }, [quantity]);

  // Hàm cập nhật giỏ hàng trong localStorage
  const updateCartInLocalStorage = (productId, newQuantity, newTotal) => {
    ...
    onCartUpdate(updatedItems); // Gọi callback để cập nhật UI
  };
  ...
  const removeFromCart = (e) => {
    e.preventDefault();
    const cart = JSON.parse(localStorage.getItem("cart")) || { items: [] };
    const updatedItems = cart.items.filter((cartItem) => cartItem.productId !== item.productId);
    localStorage.setItem("cart", JSON.stringify({ ...cart, items: updatedItems }));
    onCartUpdate(updatedItems); // Gọi callback để cập nhật UI
  };

  return (
    <div className={`sb-cart-item sb-cart-item-${index}`}>
      <div className="row align-items-center">
        {/* Sản phẩm */}
        <div className="col-lg-6">
          <Link className="sb-product" href={`/product`}>
        ...
        <div className="col-3 col-lg-1">
          <div className="sb-price-1">
            <span>Price: </span>
            {formatPrice(item.price).toLocaleString("vi-VN")} VNĐ
          </div>
        </div>
        {/* Tổng giá */}
        <div className="col-3 col-lg-1">
        ...
        {/* Xóa */}
        <div className="col-1">
          <a href="#." className="sb-remove" onClick={removeFromCart}>
            ×
          </a>
        </div>
      </div>
